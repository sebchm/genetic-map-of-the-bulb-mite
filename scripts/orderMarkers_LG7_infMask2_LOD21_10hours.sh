...
#SBATCH --mail-user=sebchm@amu.edu.pl



# sbatch 2.sexChr_LG7_infMask2_LOD21_10hours.sh 
...
gunzip -c ${DATA} | ${LM}  OrderMarkers2 data=- map=${MAP} useMorgan=1 chromosome=$CHR numThreads=40 \
        ...
        calculateIntervals=${RESULTS_DIR}/intervals/${MAP_NAME}_LOD${lod_SC}_infMask${INFMASK}_LG${CHR}_int \
        > ${RESULTS_DIR}/${MAP_NAME}_LOD${lod_SC}_infMask${INFMASK}_LG${CHR}.txt \
        2> ${RESULTS_DIR}/${MAP_NAME}_LOD${lod_SC}_infMask${INFMASK}_LG${CHR}_log.txt

        awk -vFS="\t" -vOFS="\t" '(NR==FNR){s[NR-1]=$0}(NR!=FNR){if ($1 in s) $1=s[$1];print}' ${SNP_FILE} \
        ${RESULTS_DIR}/${MAP_NAME}_LOD${lod_SC}_infMask${INFMASK}_LG${CHR}.txt > \
        ${RESULTS_DIR}/${MAP_NAME}_LOD${lod_SC}_infMask${INFMASK}_LG${CHR}_named.txt
